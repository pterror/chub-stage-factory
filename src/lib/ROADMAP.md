...
- ✅ `freeform-pipeline.ts` — full escape-hatch loop: freeform text → intent parse → oracle delta → policy → apply → render
- ✅ `render-trigger.ts` — wires trigger-fires → ContextAssembler → LlmPipeline → prose
...
- ✅ `synergy/sliding-window-chat.ts` — pairs the chatWindow primitive with Timeline so that as turns age out of the bounded verbatim window, their relevant content is captured as Timeline events (via tag-parser extraction or LLM summarization). Information persists; verbatim text doesn't. Defaults to 8 turns; the "I want a 200-turn raw history" path requires explicit author opt-in plus awareness of context cost.
...
- **Composer build pass 2026-05-27** — 33 composers + `world.ts` primitive shipped across 6 parallel agent batches. See DECISIONS.md §13 for LOC totals and reconciliation notes.
  ...
  - `focusPattern` — **SHIPPED** (`src/lib/patterns/lifecycle/focus.ts`). Now ✅.
  - `synergy/sliding-window-chat.ts` — **SHIPPED** (`src/lib/patterns/synergy/sliding-window-chat.ts`). `chat-window.ts` `@experimental` marker removed. Now ✅.
